...
%         disp([erun mrun])
        ...
        ierun=sub2ind([MRUNS EXPTRUNS],mrun,erun);
        ...
        margtc=milecost(ierun)*milestraveled(ierun)*cell2mile;        %Assumed: 250 travel days a year, roundtrip
        %%% Zones Layer %%%
        NZONES=25;
...
%         Allfarmdist=zeros(Nfarmers,1);
        ...
        coastprox=num2cell(am0(ierun)*1./exp(am_slope(ierun)*coastdist));
        ...
        Psevere=stormfreq(ierun)/100;
%         Psevere=stormprob{stormfreq(ierun)};
%         Psevere=stormfreq(ierun)*[0.013 0 0 0 0; 0.0065 0.0065 0 0 0; 0.013 0 0 0 0; 0.1364 0.0844 ...
%             0.0714 0.0065 0; 0.0065 0 0 0 0; 0.0584 0.013 0.0065 0 0];
        
        % maxPflood=0.7;
        % highrisk=30;
        risksurf=1./(1+exp((coastdist-highrisk(ierun))/10));
        Pflood=num2cell(reshape(maxPflood(ierun)*(risksurf+(1-max(max(risksurf)))),NCELLS,1));
...
%         Pimpact=cat(1,Pflood{:})*mean(Psevere,1);
        ...
        housedam=maxdam(ierun)*0.01*(10.23749-0.23462*(coastdist*cell2ft/1000)+...
            0.001649*(coastdist*cell2ft/1000).^2);
%         housedam=maxdam(ierun)*[0.05 0.1 0.25 0.5 1];
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   ZONES Layer    %%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for nf=1:Nfarmers
            ...
            if farmmindist < 0.3
                farmret=ones(length(farmacres),1)*coastvalue(ierun)*AVGFARMRETURN(ierun)-...
                    cat(1,travelcost{farmacres});
            elseif farmmindist >=0.3 && farmmindist < 1
                farmret=ones(length(farmacres),1)*midvalue(ierun)*AVGFARMRETURN(ierun)-...
                    cat(1,travelcost{farmacres});
            elseif farmmindist >= 1
                farmret=ones(length(farmacres),1)*inlandvalue(ierun)*AVGFARMRETURN(ierun)-...
                    cat(1,travelcost{farmacres});
            end
...
%         load master_dist
        ...
        CONINFO(:,6)=num2cell((ampref_min(ierun)+(ampref_max(ierun)-ampref_min(ierun))*...
...
%         load rentmodel
        ...
        while abs(deltafac) > 0.001
            ...
            Cdam(1:Nlots(TSTART),TSTART)=num2cell(Paskhouse.*housedam(cat(1,lotchoice{:,2})));
...
%                     repmat(cat(1,Pflood{cat(1,lotchoice{inewlots,2})}),1,5),2));
            ...
            for c=1:length(inewcon)
                ...
                EUnomit(inewcon(c),inewlots)=Psevere*...
...
%         ndate=datestr(date,'ddmmyy');
        savefname=sprintf('gsa_run%d_%d.mat',erun,mrun);
        parsave_gsa(savefname,...
        ...
        %         fname=sprintf('results_CHALMS_Coast_batch_%d_%d',ierun,mrun);
